from django.db import migrations, models


def gen_unique_codes(apps, schema_editor):
    Accountingclasstypes = apps.get_model('parameters', 'Accountingclasstypes')
    for i, obj in enumerate(Accountingclasstypes.objects.iterator(), start=1):
        obj.code = f'C{i:03d}'
        obj.save(update_fields=['code'])


class Migration(migrations.Migration):

    dependencies = [
        ('parameters', '0016_chartofaccounts_models'),
    ]

    operations = [
        migrations.AddField(
            model_name='accountingclasstypes',
            name='nature',
            field=models.CharField(default='devedora', max_length=50, verbose_name='Natureza contábil'),
            preserve_default=False,
        ),
        migrations.AddField(
            model_name='accountingclasstypes',
            name='code',
            field=models.CharField(blank=True, max_length=50, null=True, verbose_name='Código da classe'),
        ),
        migrations.RunPython(gen_unique_codes, reverse_code=migrations.RunPython.noop),
        migrations.AlterField(
            model_name='accountingclasstypes',
            name='code',
            field=models.CharField(max_length=50, unique=True, verbose_name='Código da classe'),
            preserve_default=False,
        ),
        migrations.AlterModelOptions(
            name='accountingclasstypes',
            options={'ordering': ['code'], 'verbose_name': '18. Classe Contábil', 'verbose_name_plural': '18. Classes Contábeis'},
        ),
    ]
